...
def chunkifyX4TextField( oldtext ):
    """
    Splits an Exfor text field into chunks, code pieces ( i.e. "(blah, blah)" ) get there own chunks and "free text" get their own chunks.  Works best on 'INSTITUTE' field.  Would be better to move to pyparsing based scheme.
    @type  oldtext: string
    @param oldtext: raw line from an Exfor file 
    @rtype: list of strings
    @return: list containing the (hopefully) parsed Exfor text field
    """
    if type(oldtext) != str: raise TypeError
...
class X4PlainField( dict ):
    ...
    def __init__( self, raw_field = [] ):
        ...
        for line in raw_field:
            ...
            if line[10] != ' ': pointer = line[10]
            if not pointer in self: self[ pointer ] = X4SubField()
    ...
    def __str__( self ): 
        ...
        for p in self.sorted_keys:
            if p != ' ': result.append( '[' + p + '] ' + str( self[ p ] ) )
            else:        result.append( str( self[ p ] ) )
        return '; '.join( result )
...
class X4ReactionField( X4PlainField ):
    ...
    def __init__( self, x ):
        ...
        for p in self:
            ...
            try:
                ...
                raise exfor_exceptions.ReactionParsingError( 'Can not parse reaction "' + d + '",\n    got error "' + str( err ) + '"\n   ' )
            self.reactions[ p ] = [ self.getMeasurementType( f[0:-1] ), f[-1] ]
            
    def getMeasurementType( self, x ):
        try:
            for i in [ '(', ')', '+', '-', '*', '/', '=', '//' ]:
                if i in x:  return exfor_reactions.X4ReactionCombination( x )
            return exfor_reactions.X4Reaction( x[ 0 ] )
        except ( exfor_exceptions.IsomerMathParsingError ), err: return exfor_reactions.X4ReactionIsomerCombination( x )
    ...
    def getEquation( self, key, schematic=False ):
        ...
        if isinstance( self.reactions[key][0], exfor_reactions.X4Reaction ): 
            if schematic: return 'rxn 0'
            else: return repr( self.reactions[key][0] )
        for thing in self.reactions[key][0].data:
            if type(thing) != str: 
                if schematic: result.append( 'rxn ' + str( i ) )
                else : result.append( repr( thing ) )
                i += 1
    ...
    def getX4ReactionList( self, key ):
        ...
        Arguments:
            ...
            * key : the EXFOR pointer (either '' or an integer)
        '''
        if isinstance( self.reactions[key][0], exfor_reactions.X4Reaction ): return [ self.reactions[key][0] ]
        result = []
        for thing in self.reactions[key][0].data:
            if isinstance( thing, exfor_reactions.X4Reaction ): result.append( thing )
        return result
    ...
    def __str__( self ):
        ...
        for j in self: 
            if j == ' ': ans.append(  str( self.reactions[ j ][0] ) )
            else: 
                ans.append( '[' + j + '] ' + str( self.reactions[ j ][0] ) )
                if self.reactions[ j ][1] != '': ans[-1] += ', ' + self.reactions[ j ][1]
        return '; '.join( ans )
...
class X4MonitorField( X4ReactionField ): 
    ...
    def __str__( self ):
        ...
        if self.reactions == {}: return ans
        for j in self: 
            if j == ' ': ans.append(  str( self.reactions[ j ] ) )
            else: 
                ans.append( '[' + j + '] ' + str( self.reactions[ j ] ) )
        return '; '.join( ans )
  
  
pubTypeMap = { 'A':'Abstract', 'K':'Abstract of Journal', 'J':'Journal', 'C':'Conf. Proc.', 'S':'Conf. Proc.', 'P':'Prog. Report', 'R':'Lab Report', 'B':'Book', 'W':'Private Comm.', 'T':'Thesis', 'X':'Preprint' }
        ...
        try:
            for p in self:
                ...
                for line in self[ p ]:
                    if line[0:2] == '((': 
                        ...
                        refs[-1] += line.replace( '\n', '' )
                    if '))' in line: intautology = False
                for xx in refs:
                    if hastautology and '=' in xx:
                        try:
                            xref = exfor_reference.x4refcodetautology.parseString( xx ).asList()
                        except pyparsing.ParseException, err:
                            raise exfor_exceptions.ReferenceParsingError( 'Can not parse reference "' + xx + '",\n    got error "' + str( err ) + '"\n   ' )
                        refcomment = xref[-1]
                        for xxx in xref[0:-1]:
                            try: 
                                if xxx != '=': self.refs[ p ].append( ( exfor_reference.X4ReferenceCode( self.flatten( xxx ) ), refcomment ) )
                            except ( KeyError, pyparsing.ParseException, LookupError, AssertionError ), err:
                                raise exfor_exceptions.ReferenceParsingError( 'Can not parse reference "' + xx + '",\n    got error "' + str( err ) + '"\n   ' )
                    else:
            ...
            for p in self.refs:
                if isinstance ( self.refs[p][0][0], exfor_reference.X4ReferenceCode ):
                    ...
                    break
        except exfor_exceptions.ReferenceParsingError, err:
                    self.reftype = '?'
    ...
    def __init__( self, x ):
        ...
        for p in self:
            try:
                ...
                raise exfor_exceptions.InstituteParsingError( 'Can not parse institute "' + str( x ) + '",\n    got error "' + str( err ) + '"\n   ' )
            for i in il:
                tok = i[0][0]
