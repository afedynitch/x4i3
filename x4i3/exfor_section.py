...
class X4BibSection( X4Section, dict ):
    ...
    def __init__( self, unprocessed_section = [] ):
        ...
        for field in self.chunkify( unprocessed_section ):
            ...
            if   ftag == 'REACTION' : self[ ftag ] = exfor_field.X4ReactionField( field ) 
            elif ftag == 'MONITOR':   self[ ftag ] = exfor_field.X4MonitorField( field )
            elif ftag == 'REFERENCE': self[ ftag ] = exfor_field.X4ReferenceField( field )
            elif ftag == 'AUTHOR':    self[ ftag ] = exfor_field.X4AuthorField( field )
            elif ftag == 'TITLE':     self[ ftag ] = exfor_field.X4TitleField( field )
            elif ftag == 'INSTITUTE': self[ ftag ] = exfor_field.X4InstituteField( field )
            else:                     self[ ftag ] = exfor_field.X4PlainField( field )
        
    def __str__( self ):
        ans = self.tag.ljust( 11 ) + str( len( self ) ).rjust( 11 ) + str( self.totalLen() ).rjust( 11 ) + '\n'
        for i in self.sorted_keys: ans += str( self[i] ) + '\n'
    ...
    def __repr__( self ):
        ...
        for i in self.sorted_keys: ans += repr( self[i] ) + '\n'
    ...
    def chunkify( self, oldsection ):
        ...
        """
        if type( oldsection ) != list: raise TypeError
        section = []
        first=True
        field=[]     
...
class X4BibMetaData:
    ...
    def __init__( self, **kw ):
        if kw.has_key( 'bib' ):
            bib = kw[ 'bib' ]
            if not isinstance( bib, X4BibSection ): raise TypeError( "X4BibMetaData.__init__ takes an X4BibSection as the argument, got an " + str( type( entry ) ) )
            try:             self.author = bib[ 'AUTHOR' ].authors
            except KeyError: self.author = 'None'
            try:             self.institute = bib[ 'INSTITUTE' ]
            except KeyError: self.institute = 'None'
    ...
    def citation( self ): 
        ...
        else: author_string = ', '.join( self.author )
        return author_string + ", " + str( self.reference ) + ";  Data taken from the EXFOR database, file EXFOR " + self.subent + " dated " + str( self.reference.date ) + ", retrieved from the IAEA Nuclear Data Services website."
    ...
    def legend( self ):
        ...
        else : return '(' + self.year + ') ' + ', '.join( self.author )
...
class X4DataSection( X4Section ):
    ...
    def __init__( self, tag, unprocessed_section = [] ):
        ...
        self.VERBOSELEVEL = 0
        self.LPR = ( self.numcols - 1 ) / 6 + 1 # number lines in a DATA block per row of real data!
        if len( unprocessed_section ) != 2 + ( self.numrows + 2 ) * self.LPR:
            if len( unprocessed_section ) == 2 + ( self.numrows ) * self.LPR:
                self.numrows -= 2 
                if self.VERBOSELEVEL > 1: print 'Column headings were included in datafield size count'
            elif len( unprocessed_section ) == 2 + self.numrows:
                self.numrows = self.numrows / self.LPR - 2
                if self.VERBOSELEVEL > 1: print 'Datafield size count was set to number of lines in section'
            else:
        ...
        iline = 0
        collapsed_section = []
        for i in range( 1, len( unprocessed_section ) - 1, self.LPR ): #skip begin & end of section tags
            row = ''
        ...
        for j in range( self.numcols ):
            ...
            if len( self.labels[ -1 ] ) == 11: #then it has a pointer because there is something in the 11th column of the field
                p = self.labels[ -1 ][ 10 ]
                if not p in self.pointers: self.pointers[ p ] = []
    ...
    def __str__( self ):
        ...
        for i in self.labels: result += i.ljust( 11 )
        result += '\n'
        for i in self.units:  result += i.ljust( 11 )
        result+='\n'
        for i in self.data:
            for j in i:  result += str( j ).ljust( 11 )
            result += '\n'
