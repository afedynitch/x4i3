...
class TestX4ReactionField( unittest.TestCase ):
    ...
    def test_str( self ):
        self.assertEqual( str( self.field ), '(( Resonance integral for 64Zn(n,gamma)65Zn )/( Cross section for 64Zn(n,gamma)65Zn ))' )
    ...
    def test_you_gotta_be_kidding( self ):
        ...
        self.assertEqual( str( testField ), '(( Cross section for 197Au(n,2n)196m2Au )/( (( Cross section for 197Au(n,2n)196m0Au )+( Cross section for 197Au(n,2n)196mAu )) ))' )

    def test_no_you_really_gotta_be_kidding( self ):
        testFieldString = '''REACTION   (48-CD-116(N,2N)48-CD-115-M/G,,SIG/RAT)'''
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '(( Cross section ratio for 116Cd(n,2n)115mCd )/( Cross section ratio for 116Cd(n,2n)115m0Cd ))' )

    def test_please_stop( self ):
        testFieldString = '''REACTION  1(40-ZR-88(N,N+P)39-Y-87-M/T,,SIG/RAT)
          2(40-ZR-88(N,N+P)39-Y-87,,SIG)'''
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '[1] (( Cross section ratio for 88Zr(n,n+p)87mY )/( Cross section ratio for 88Zr(n,n+p)87mY )); [2] Cross section for 88Zr(n,n+p)87Y' )

    def test_oh_god_no( self ):
        testFieldString = 'REACTION   (96-CM-248(10-NE-22,5N)106-SG-265-M1+M2,,SIG)'
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '(( Cross section for 248Cm(22Ne,5n)265mSg )+( Cross section for 248Cm(22Ne,5n)265m2Sg ))' )
        ...
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '[1] Cross section for 178m2Hf(n,gamma)179m0Hf; [3] (( Cross section ratio for 178m2Hf(n,gamma)179m2Hf )/( Cross section ratio for 178m2Hf(n,gamma)179m0Hf )); [2] Resonance integral for 178m2Hf(n,gamma)179m0Hf; [5] Resonance integral for 178m2Hf(n,gamma)179m2Hf; [4] Cross section for 178m2Hf(n,gamma)179m2Hf' )
        ...
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '(( Cross section for 178m2Hf(n,gamma)179m0Hf )+( Cross section for 178m2Hf(n,gamma)179m2Hf ))' )
        testFieldString = '''REACTION   ((94-PU-239(N,F),,SIG)/
           (92-U-235(N,F),,SIG))'''
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '(( Cross section for 239Pu(n,Fission) )/( Cross section for 235U(n,Fission) ))') 
        testFieldString = '''REACTION   ((92-U-233(N,F),,SIG)/
           (92-U-235(N,F),,SIG))'''
        testField = exfor_field.X4ReactionField( testFieldString.split( '\n' ) )
        self.assertEqual( str( testField ), '(( Cross section for 233U(n,Fission) )/( Cross section for 235U(n,Fission) ))' ) 
...
class TestX4ReactionFieldPointer( unittest.TestCase ):
    ...
    def test_str( self ):
        self.assertEqual( str( self.field ), '[1] (( Resonance integral for 64Zn(n,gamma)65Zn )/( Cross section for 64Zn(n,gamma)65Zn )); [2] Resonance integral for 64Zn(n,gamma)65Zn, Reduced resonance integral and ratio to thermal absorption cross section' )
...
class TestX4MonitorField( unittest.TestCase ):
    ...
    def test_multiline( self ):
        ...
        self.assertEqual( str( X.reactions ),"{' ': [((62-SM-147(N,A)60-ND-144,SIG), 'Data were normalized over the interval 300-2500 eV to the data given in monitor reference', None)]}" )
        self.assertEqual( str( X ), "[((62-SM-147(N,A)60-ND-144,SIG), 'Data were normalized over the interval 300-2500 eV to the data given in monitor reference', None)]"  )
        self.assertEqual( X.reactions.keys(), [ ' ' ] )
        self.assertEqual( X.reactions[' '][0][1], 'Data were normalized over the interval 300-2500 eV to the data given in monitor reference' )
    ...
    def test_multiline_boring( self ):
        ...
        self.assertEqual( str( X.reactions ), "{' ': [(None, 'Reactor grade graphite samples with thickness of 7 cm and 13 cm were useded for monitoring', None)]}" )
        self.assertEqual( str( X ), "[(None, 'Reactor grade graphite samples with thickness of 7 cm and 13 cm were useded for monitoring', None)]" )

    def test_two_monitors( self ):
        X = exfor_field.X4MonitorField( self.long_example.split( '\n' ) )
        self.assertEqual( repr( X ), self.long_example )
        self.assertEqual( str( X.reactions ), "{' ': [((79-AU-197(N,G)79-AU-198,RI), '', 'MONIT1'), ((27-CO-59(N,G)27-CO-60,RI), '', 'MONIT2')]}" )
        self.assertEqual( str( X ), "[((79-AU-197(N,G)79-AU-198,RI), '', 'MONIT1'), ((27-CO-59(N,G)27-CO-60,RI), '', 'MONIT2')]" )
    ...
    def test_three_monitors( self ):
        ...
        self.assertEqual( str( X.reactions ),"{' ': [((22-TI-0(D,X+23-V-48),SIG), '21 MeV', None), ((13-AL-27(D,X+11-NA-22),SIG), '40 MeV', None), ((13-AL-27(D,X+11-NA-24),SIG), '40 MeV', None)]}" )
        self.assertEqual( X.reactions.keys(), [ ' ' ] )
    ...
    def test_two_monitors_many_lines( self ):
        ...
        self.assertEqual( str( X.reactions ), "{' ': [((79-AU-197(N,G)79-AU-198,SIG,MXW), 'Monitor in form of alloy of 0.134% in thin Al metal foil', 'MONIT1'), ((27-CO-59(N,G)27-CO-60,SIG,MXW), 'Monitor in form of dilute  alloy  of 0.438% in thin Al metal foil', 'MONIT3')]}" )
        self.assertEqual( str( X ), "[((79-AU-197(N,G)79-AU-198,SIG,MXW), 'Monitor in form of alloy of 0.134% in thin Al metal foil', 'MONIT1'), ((27-CO-59(N,G)27-CO-60,SIG,MXW), 'Monitor in form of dilute  alloy  of 0.438% in thin Al metal foil', 'MONIT3')]" )
...
class TestX4AuthorField( unittest.TestCase ):
    ...
    def test_really_long( self ):
        ...
        self.assertEqual( exfor_field.X4AuthorField( self.really_long_example.split( '\n' ) ).author_family_names, ['Budzanowski', 'Chatterjee', 'Gebel', 'Hawranek', 'Jahn', 'Jha', 'Kilian', 'Kliczewski', 'Kirillov', 'Kirillov', 'Kolev', 'Kravcikova', 'Lesiak', 'Lieb', 'Machner', 'Magiera', 'Maier', 'Martinska', 'Nedev', 'Piskunov', 'Prasuhn', 'Protic', 'Ritman', 'Von Rossen', 'Roy', 'Sitnik', 'Siudak', 'Tsenov', 'Urban', 'Vankova', 'Wilkin'] )
