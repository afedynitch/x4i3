...
exfor_entry module 
"""
import exfor_subentry, exfor_dataset, exfor_exceptions, os
...
def x4EntryFactory( enum, subentsList=None, rawEntry=False ):
    ...
    '''
    if len(enum) != 5: raise ValueError( "A valid EXFOR ENTRY is a string with exactly 5 characters")    
    result = [] # the entry, split into subentries

    entry = open( os.sep.join( [ fullDBPath, enum[:3], enum + '.x4' ] ), mode = 'rU' ).readlines()
    subent = ''
    for line in entry:
        if line.startswith('ENTRY') or line.startswith('ENDENTRY') or line.startswith('NOSUBENT'): pass
        elif line.startswith( 'SUBENT' ): subent = line
        else: subent += line
        if line.startswith( 'ENDSUBENT' ):
            if subentsList == None or subent[14:22].strip() in subentsList: result.append( subent )
            subent = ''
...
def x4DictionaryEntryFactory( enum, subentsList=None, rawEntry=False ):
    ...
    if len(enum) != 5: raise ValueError( "A valid EXFOR ENTRY is a string with exactly 5 characters: ", enum, len(enum))    
    result = [] # the entry, split into subentries
...
def extractX4EntryIndex( entry ):
    ...
    for line in entry:
        ...
        if line.split()[0] == "ENTRY":  return line[14:22].strip()
        if line.split()[0] == "SUBENT": return line[14:22][0:5] #this'll work too
    return None
...
class X4Entry( dict ):
    ...
    def __getitem__( self, i ):
        ...
        if type( i ) == int: i = str( i )
        if len( i ) == 8:   return dict.__getitem__( self, i )
        elif len( i ) <= 3: return dict.__getitem__( self, self.accnum + i.zfill( 3 ) )
        else :              raise KeyError( 'Invalid SubEntry index :' + i )
    ...
    def getDataSets( self ):
        datasets = {}
        if self.deleted(): return datasets
        ...
        if self[ 1 ].has_key( 'BIB' ): 
            ...
            if self[ 1 ][ 'BIB' ].has_key( 'REACTION' ): main_reactions = self[ 1 ][ 'BIB' ][ 'REACTION' ].reactions
            if self[ 1 ][ 'BIB' ].has_key( 'MONITOR' ): main_monitors = self[ 1 ][ 'BIB' ][ 'MONITOR' ].reactions
        if self[ 1 ].has_key( 'COMMON' ): main_common = self[ 1 ][ 'COMMON' ]
        ...
        for subent in self.values():
            ...
            if not subent.has_key( 'DATA' ): continue
            ...
            if subent.has_key( 'BIB' ): 
                ...
                if subent[ 'BIB' ].has_key( 'REACTION' ): subent_reactions = subent[ 'BIB' ][ 'REACTION' ].reactions
                if subent[ 'BIB' ].has_key( 'MONITOR' ): subent_monitors = subent[ 'BIB' ][ 'MONITOR' ].reactions
            if subent.has_key( 'COMMON' ): subent_common = subent[ 'COMMON' ]
            ...
            for p in main_reactions:
                ...
                try:    
                    if main_monitors.has_key(p): datasets[ k ] = exfor_dataset.X4DataSetFactory( quant = main_reactions[ p ][0].quantity, meta = meta_list, common = common_list, reaction = main_reactions[ p ], monitor = main_monitors[ p ], data = subent[ 'DATA' ], pointer = p )
                    else:                        datasets[ k ] = exfor_dataset.X4DataSetFactory( quant = main_reactions[ p ][0].quantity, meta = meta_list, common = common_list, reaction = main_reactions[ p ], data = subent[ 'DATA' ], pointer = p )
            ...
            for p in subent_reactions:
                k = ( self.accnum, subent.accnum, p )
                try:    
                    if subent_monitors.has_key(p): datasets[ k ] = exfor_dataset.X4DataSetFactory( quant = subent_reactions[ p ][0].quantity, meta = meta_list, common = common_list, reaction = subent_reactions[ p ], monitor = subent_monitors[ p ], data = subent[ 'DATA' ], pointer = p )
                    else:                          datasets[ k ] = exfor_dataset.X4DataSetFactory( quant = subent_reactions[ p ][0].quantity, meta = meta_list, common = common_list, reaction = subent_reactions[ p ], data = subent[ 'DATA' ], pointer = p )
                except NotImplementedError, err : 
                    self.errors[ k ] = 'Encountered NotImplementedError: ' + str( err )
                except IndexError, err:
                    self.errors[ k ] = 'Encountered IndexError:' + str( err )
...
class X4EntryMetaData:
    ...
    def __init__( self, entry ):
        if not isinstance( entry, X4Entry ): raise TypeError( "X4EntryMetaData.__init__ takes an X4Entry as the argument, got an " + str( type( entry ) ) )
    ...
    def xmgraceHeader( self ):
        if self.deleted :return 'ENTRY ' + str( self.accnum ) + ' DELETED'
