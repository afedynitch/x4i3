...
EntryLetterConversion    = { '10':'A','11':'B','12':'C','13':'D','14':'E','15':'F','16':'G','21':'L','22':'M','24':'O','25':'P','27':'R','28':'S','29':'T','31':'V' }
revEntryLetterConversion = { 'A':'10','B':'11','C':'12','D':'13','E':'14','F':'15','G':'16','L':'21','M':'22','O':'24','P':'25','R':'27','S':'28','T':'29','V':'31' }
...
class X4DBManager:
    ...
    def __getitem__( self, i ):
        ...
        """
        i = self.__fixkey__( i )
        if len( i ) == 5: return self.retrieve( ENTRY = i )
    ...
    def __contains__( self, i ):
        ...
        """
        i = self.__fixkey__( i )
        if len( i ) == 5: return len( self.query( ENTRY = i ) ) > 0
    ...
    def decompress_entry( self, s ):
        ...
        if not zipfile.is_zipfile( 'davestmpfile.zip' ): raise Exception( "Cannot decompress entry "+str(s[1]) )
        news = ( s[0], ''.join( os.popen( 'unzip -c davestmpfile.zip' ).readlines()[2:] ) )
        if os.path.exists( s[0]+'.txt' ): os.remove( s[0]+'.txt' )
        if os.path.exists( 'davestmpfile.zip' ): os.remove( 'davestmpfile.zip' )
        return news
    ...
    def run_sql_query( self, table, column, condition = None, VERBOSE = False ):
        ...
        if condition != None: q += " where " + condition
        if VERBOSE: print(q)
        return self.CURSOR.execute( q )

    def query( self, author = None, reaction = None, target = None, projectile = None, quantity = None, product = None, MF = None, MT = None, C = None, S = None, I = None, SUBENT = None, ENTRY = None ):
    ...
    def retrieve( self, author = None, reaction = None, target = None, projectile = None, quantity = None, product = None, MF = None, MT = None, C = None, S = None, I = None, SUBENT = None, ENTRY = None ):
...
class X4DBManagerPlainFS( X4DBManager ):
    ...
    def __init__( self, **kw ): 
        ...
        if 'database' in kw: del kw[ 'database' ] 
        if 'datapath' in kw: del kw[ 'datapath' ] 
        import sqlite3
    ...
    def query( self, author = None, reaction = None, target = None, projectile = None, quantity = None, product = None, MF = None, MT = None, C = None, S = None, I = None, SUBENT = None, ENTRY = None ):
        ...
        for key in [ 'projectile', 'product', 'MF', 'MT', 'C', 'S', 'I' ]:
            if eval( key ) != None: 
                try: raise NotImplementedError( "Retrieval critria " + key + " not implemented yet" )
                except NotImplementedError: pass

        criteria = []

        # Search for matching authors
        if author != None:   criteria.append( "author = '%s' " % ( author.capitalize() ) )

        # Search for matching target
        if target != None:   criteria.append( "target = '%s' " % ( target.upper() ) )
            
        # Search for matching reaction
        if reaction != None: 
            if reaction.find('*') != -1:
                criteria.append( "reaction LIKE '%s' " % ( reaction.replace('*','%').upper() ) )
            else:  
                criteria.append( "reaction = '%s' " % ( reaction.upper() ) )
            
        # Search for matching reaction
        if projectile != None: criteria.append( "projectile = '%s' " % ( projectile.upper() ) )
            
        # Search for matching quantity
        if quantity != None: 
            if quantity == 'CS': criteria.append( "quantity = '%s' " % ( "SIG" ) )
            else: criteria.append( "quantity = '%s' " % ( quantity.upper() ) )
            
        # Search for matching SUBENTRY
        if SUBENT != None:   criteria.append( "subent = '%s' " % ( SUBENT ) )

        # Search for matching ENTRY
        if ENTRY != None:    criteria.append( "entry = '%s' " % ( ENTRY ) )
        ...
        for r in result_list:
            e = r[0:5]
            if not e in result_map:      result_map[ e ] = [ e + '001' ]
            if not r in result_map[ e ]: result_map[ e ].append( r )
        
        return result_map
        
    def retrieve( self, author = None, reaction = None, target = None, projectile = None, quantity = None, product = None, MF = None, MT = None, C = None, S = None, I = None, SUBENT = None, ENTRY = None, rawEntry = False ):
        '''Execute a query, matching the criteria specified.          
        This function returns a dictionary with the following structure: { ENTRY#0:[ SUBENT001, SUBENT#1, SUBENT#2, ... ], ... }.
        Here ENTRY#0 is the entry number whose subentries match the query.  The SUBENT001 is the documentation subentry itself, which is 
        always included, and SUBENT#1, ... are the subentries themselves matching the search criteria.
