...
def extractX4SubEntryIndex( subentry ):
    ...
    for line in subentry:
        ...
        if sline != [] and sline[0] == "SUBENT": return line[14:22]
    return None
...
def extractX4SectionType( section ):
    ...
    for line in section:
        tag = line[0:10].strip()
        if tag in ('BIB', 'DATA','COMMON'): return tag
    return None
...
class X4SubEntry( dict ):
    ...
    def __init__( self, unprocessed_subentry ):
        if type( unprocessed_subentry ) != list: unprocessed_subentry = unprocessed_subentry.split( '\n' )
        ...
        for section in self.chunkify( unprocessed_subentry ):
            ...
            if tag == 'BIB' :                 self[ tag ] = exfor_section.X4BibSection( section )
            elif tag in ( 'DATA', 'COMMON' ): self[ tag ] = exfor_section.X4DataSection( tag, section )
            else:                             raise KeyError( tag + " is not a valid EXFOR Section name" )
        self.hascommon = self.has_key( 'COMMON' ) and self[ 'COMMON' ] != None
