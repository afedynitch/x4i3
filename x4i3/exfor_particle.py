...
# Global data
#------------------------------------------------------
x4ZMap = { '0':0, 'G':0, 'AR':0, 'N':0, 'P':1, 'AP':-1,  'AN':0, 'H':1, 'D':1, 'T':1, 'A':2, 'HE3':2, 'HE2':2, 'PI':0, 'PI0':0, 'P0':0, 'PIP':+1, 'ETA':0, 'PIN':-1, 'KP':+1, 'KN':-1, 'E':-1, 'B':-1, 'B+':+1, 'B-':-1, 'XR':0 }
x4AMap = { '0':0, 'G':0, 'AR':0, 'N':1, 'P':1, 'AP':-1, 'AN':-1, 'H':1, 'D':2, 'T':3, 'A':4, 'HE3':3, 'HE2':2, 'PI':0, 'PI0':0, 'P0':0, 'PIP':0, 'ETA':0, 'PIN':0, 'KP':0, 'KN':0, 'E':0, 'B':0, 'B+':0, 'B-':0, 'XR':0 }
x4Chemicals = { 'OXI':'Oxide', 'CMP':'Compound', 'WTR':'Water', 'D2O':'Deuterated_Water' }
x4Hadrons = { 'N':'n', 'P':'p', 'AP':'anti-p', 'AN':'anti-n', 'PI0':"pi0", 'P0':"pi0", 'PIP':"pi+", 'PIN':"pi-", 'KP':"K+", 'KN':"K-", 'ETA':'eta' }
...
class X4ParticleBase:
    ...
    def __init__( self, x ):
        ...
        if not isinstance( x, pyparsing.ParseResults ): raise TypeError( "__init__ takes pyparsing.ParseResults as argument, got " + str( type( x ) ) + ":" + str( x ) )
    ...
    def parse( self, x ): raise NotImplementedError( "Override this function in derived classes" )
    
    def getZ( self ):
        if self.num == 'X': return int( self.Z )
        return self.num * int( self.Z )
        
    def getA( self ):
        if self.num == 'X': return int( self.A )
        return self.num * int( self.A )  
        
    def asList( self ): 
        if type( self.parse_results ) == list: l = self.parse_results
        else:                                  l = self.parse_results.asList()
...
class X4Particle( X4ParticleBase ):
    ...
    def __init__( self, x ):
        if x == '0': # sponaneous process, must catch it somehow
            ...
            else: raise TypeError( str( type( x ) ) + ' is not a valid type for X4particle.__init__, x = ' + str( x ) )
            if len( x ) > 1:
                self.num = int( x[0])
                self.sym = x[1]
            else:
    ...
    def parse( self, x ): 
        try: return x4particle.parseString( x )
        except pyparsing.ParseException, err: raise ParticleParsingError( 'Can not parse particle "' + x + '",\n    got error "' + str( err ) + '"\n   ' )
        
    def prettyStyle( self ):
        if self.num > 1: ans = str( self.num )
        else: ans = ''
        if self.sym in x4Hadrons: return x4Hadrons[ self.sym ]
        if len( self.sym ) == 1:
            if   self.sym == 'A': ans += 'alpha'
            elif self.sym == 'G': ans += 'gamma'
            elif self.sym == '0': ans += 'None'
            elif self.sym[0] == 'B': ans += 'beta' + self.sym[1:]
            else : ans += self.sym.lower()
        else: ans += self.sym.capitalize()
        return ans
        
    def exforStyle(self):
        if self.num > 1: return str( self.num ) + self.sym
        else: return self.sym
...
class X4Nucleus( X4ParticleBase ):
    ...
    def parse( self, x ): 
        try: return x4nucleus.parseString( x )
        except pyparsing.ParseException, err: raise ParticleParsingError( 'Can not parse nucleus "' + x + '",\n    got error "' + str( err ) + '"\n   ' )
        
    def endlZAStyle( self ):
        return 'za' + self.Z.zfill( 3 ) + self.A.zfill( 3 )
        
    def exforStyle( self ):
        if self.sym == 'Nn': return 'n'
...
class X4Isomer( X4Nucleus ):
    ...
    def __init__( self, x, IgnoreIsomerMath = False ):
        ...
        for i in range( 4, len( x ) ):
            if x[i].lower() == 'g': self.isomers.append( '0' )
            elif x[i].lower() == 'm' and len( x ) > i + 1: pass 
            elif x[i].isdigit(): self.isomers.append( x[i] )
            elif x[i].lower() == 't': pass
            else: self.isomers.append( '1' )
            if ( ( 'G' in x[4:] or 'T' in x[4:] ) and 'M' in x[4:] ) or x[4:].count( 'M' ) > 1: 
                if not IgnoreIsomerMath: raise IsomerMathParsingError( "Math with multiple states in isomer element, who thought of that format?  Found: " + str( x ) )
                else:                    self.isomers += ''.join( x[4:] )
                
    def endlZAStyle( self ):
        za = X4Nucleus.endlZAStyle( self )
        if len( self.isomers ) == 1 and self.isomers[0] == '1':  return za + 'm'
    ...
    def prettyStyle( self ):
        if len( self.isomers ) == 1 and self.isomers[0] == '1': return self.A + 'm' + self.sym
        else: return self.A + 'm' + ','.join( self.isomers ) + self.sym
...
class X4ChemicalCompound( X4ParticleBase ):
    ...
    def parse( self, x ): 
        try: return x4chemical_compound.parseString( x )
        except pyparsing.ParseException, err: raise ParticleParsingError( 'Can not parse chemical compound "' + x + '",\n    got error "' + str( err ) + '"\n   ' )
