...
x4basicparticle = Optional(Literal('X')) + eval('^'.join(['Literal( \"'+x+'\" )' for x in X4DictionaryServer()["Particles"].keys()]))
x4particle   = Combine(Word(alphas)+Optional(Word(nums)))
x4element    = Word( capsStar, alphas, min=1, max=2 )^Literal( "PI" )^Literal( "K0" )^Literal( "PIM" )^Literal( "PIP" )^Literal( "P0" )
...
x4residual   = Group(x4chemical_compound|x4nucleus)^Literal("ELEM/MASS")^Literal("MASS")^Literal("ELEM")^Literal("NPART")
...
x4isomerproduct    = Group(x4isomermath) ^ x4basicparticle ^ x4particle ^ Group(Optional(Word(nums))+x4particle) ^ Literal('0')
