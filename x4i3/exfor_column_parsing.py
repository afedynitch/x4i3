...
energyUnitConversionFactors          = { 'GEV'     : 1.0e+3, 'MEV'       : 1.0,         'KEV'        : 1.0e-3, 'EV'       : 1.0e-6,   'MILLI-EV' : 1.0e-9,   'MeV' : 1.0, 'K' : 8.621738e-11 }
...
energyDistUnitConversionFactors      = { '1/GEV'   : 1.0e-3, '1/MEV'     : 1.0,            '1/KEV'   : 1.0e+3,      '1/EV': 1.0e+6, '1/MILLI-EV' : 1.0e+9, '1/MeV' : 1.0 }
...
def averageColumns( x, y ): 
    ...
    for i in range( max( len(x), len(y) ) ):
        if x[i] != None:
            if y[i] == None: ans.append( x[i] )
            else : ans.append( 0.5 * ( x[i] + y[i] ) )
        else:
            if y[i] == None: ans.append( None )
            else: ans.append( y[i] )
    return ans
...
class X4ColumnParser:
    ...
    def isMatch( self, i, data ):
        result = True
        if ( i < 0 ) or ( i >= data.numcols() ): return False
        if self.match_labels != None: result = result and ( data.labels[i] in self.match_labels )
        if self.match_units  != None: result = result and ( data.units[i] in self.match_units )
        return result
    def firstMatch( self, data ):
        for i in range( data.numcols() ):
            if self.isMatch( i, data ): return i
        return -1
    def allMatches( self, data ):
        match_list=[]
        for i in range( data.numcols() ):
            if self.isMatch( i, data ): match_list.append( i )
        return match_list
    def getConversion( self, units ):
        ...
        if type( units ) != str: raise TypeError
        for unit_map in unit_map_of_maps.items():
            if units in unit_map[1].keys(): return unit_map[1][units], unit_map[0]
        return 1.0, units
    def getColumn( self, icol, data ):
        if not self.isMatch( icol, data ): return [ None ] * ( data.numrows() + 2 )
        ...
        for irow in range( data.numrows() ):
            ...
            if col[-1] != None: col[-1] = col[-1] * units[0] * self.scale_factor + self.off_set
        return col

class X4AngleColumnParser( X4ColumnParser ):
    def getColumn( self, icol, data ):
        # If in either radians or degrees, can handle using base class
        if not 'COS' in data.labels[ icol ]: return X4ColumnParser.getColumn( self, icol, data )
        ...
        for irow in range( data.numrows() ):
            col.append( data.data[irow][icol] )
            if col[-1] != None: col[-1] = math.acos( col[-1] * self.scale_factor + self.off_set ) * units[0]
        return col
    ...
    def getError( self, data ):
        ...
        if err[1] == 'PER-CENT':
            ...
            for i in range( 2, len( col ) ):
                if col[i] != None and err[i] != None: err[i] = col[i] * err[i] / 100.0
                else: err[i] = None
...
class X4ConstantPercentColumnPair( X4MissingErrorColumnPair ):
    ...
    def getError( self, data ):
        ...
        col = self.getValue( data )
        for i in range( 2, len( col ) ):
            if col[i] != None: col[i] = col[i] * self.percentError / 100.0
...
class X4HighLowColumnPair( X4IndependentColumnPair ):
    ...
    def getValue( self, data ):
        if not self.isMatch( data ): return self.getDummyColumn( data )
        self.set_icols( data )
        if self.column1Parser != None: col1 = self.column1Parser.getColumn( self.icol1, data )
        else : col1 = self.getDummyColumn( data )
        if self.column2Parser != None: col2 = self.column2Parser.getColumn( self.icol2, data )
        else : col2 = self.getDummyColumn( data )
        ans = [ None, None ]
        for i in [ 0, 1 ]:
        ...
        for i in range( 2, data.numrows() + 2 ):
            ...
            if x1==None and x2==None: ans.append(None)
            elif x1==None and x2!=None: ans.append(0.5*x2)
            elif x1!=None and x2==None: ans.append(None) #'>'+str(x1)
            else : ans.append(0.5*(x1+x2))
        return ans
    def getError( self, data  ):
        if not self.isMatch( data ): return self.getDummyColumn(data)
        self.set_icols( data )
        ...
        for i in range(2,data.numrows()+2):
            ...
            if x1==None or x2==None: ans.append(None)
            elif x1==None and x2!=None: ans.append(0.5*x2)
            elif x1!=None and x2==None: ans.append(None)
            else : ans.append(0.5*abs(x1-x2))
...
class X4HighMidLowColumnPair( X4IndependentColumnPair ):
    ...
    def getValue( self, data ):
        ...
        if self.column3Parser!=None: col3=self.column3Parser.getColumn( self.icol3, data )
        else : col3 = self.getDummyColumn(data)
        ans = [ None, None ]        
        for i in [ 0, 1 ]:
        ...
        for i in range( 2, data.numrows() + 2 ):
            ...
            if x1 == None or x2 == None or x3 == None: ans.append( None )
            else : ans.append( 0.5 * ( ( x1 - x2 ) + ( x1 + x3 ) ) )
        return ans
    def getError( self, data ):
        if not self.isMatch( data ): return self.getDummyColumn(data)
        self.set_icols( data )
        ...
        for i in range(2,data.numrows()+2):
            ...
            if x1==None or x2==None or x3==None: ans.append(None)
            else : ans.append(abs(0.5*((x1-x2)-(x1+x3))))
        return map(absOrNone,ans)
...
class X4AddErrorBarsColumnPair( X4HighMidLowColumnPair ):
    ...
    def getError( self, data ):
        ...
        if 'PER-CENT' in [col2[1],col3[1]]:
            ...
            ans[1]=col1[1]
            for i in range(2,data.numrows()+2):
                try: col2[i]=col1[i]*col2[i]/100.0
                except: col2[i] = None
                try: col3[i]=col1[i]*col3[i]/100.0
                except: col3[i] = None
        else :
        ...
        for i in range(2,data.numrows()+2):
            x2=col2[i]
            x3=col3[i]
            if x2==None and x3==None: ans.append(None)
            else : 
                if x2==None: x2=0.0
                if x3==None: x3=0.0
...
class X4BarnsSqrtEColumnPair( X4IndependentColumnPair ):
    ...
    def energyColumn( self, data ):
        return reduce( condenseColumn, [ i.getValue(data) for i in energyParserList ] )
    def set_icols( self, data ):
        self.icol2=self.column2Parser.firstMatch( data )
        self.icol3=self.column3Parser.firstMatch( data )
    def isMatch( self, data ):
    ...
    def getValue( self, data ):
        ...
        for i in range(2,data.numrows()+2):
            ...
            if x1==None or x2==None: ans.append(None)
            else : ans.append( x2/math.sqrt(x1*1e6) )
        return ans
    def getError( self, data ):
        if not self.isMatch(data ): return self.getDummyColumn(data)
        self.set_icols(data )
        ...
        for i in range(2,data.numrows()+2):
            ...
            if x1==None or x3==None: ans.append(None)
            else : ans.append( x3/math.sqrt(x1*1e6) )
        return map(absOrNone,ans)
...
incidentEnergyParserList = [\
    ...
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'EN'+s for s in variableSuffix ], match_units = energyUnits ),\
        X4ColumnParser( match_labels = [ 'EN'+s for s in resolutionFWSuffix ], match_units = energyUnits + percentUnits, scale_factor = 0.5 )\
    ),\
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'EN' + s for s in variableSuffix ], match_units = energyUnits ),\
    ...
    X4MissingErrorColumnPair(\
        X4ColumnParser( match_labels = [ 'EN' + s for s in variableSuffix ] + baseMomKeys, match_units = energyUnits + momUnits ),\
        None,\
    ),\
]
...
outgoingEnergyParserList = [\
    ...
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'E'+s for s in variableSuffix ], match_units = energyUnits ),\
        X4ColumnParser( match_labels = [ 'E'+s for s in resolutionFWSuffix ], match_units = energyUnits + percentUnits, scale_factor = 0.5 )\
    ),\
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'E' + s for s in variableSuffix ], match_units = energyUnits ),\
    ...
    X4MissingErrorColumnPair(\
        X4ColumnParser( match_labels = [ 'E' + s for s in variableSuffix ] + baseMomKeys, match_units = energyUnits + momUnits ),\
        None,\
    ),\
]
...
tempParserList = [\
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'KT' + s for s in variableSuffix + shiftSuffix], match_units = energyUnits+tempUnits ),\
        X4ColumnParser( match_labels = [ 'KT' + s for s in errorSuffix ], match_units = energyUnits+tempUnits + percentUnits ),\
     ),\
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'KT' + s for s in variableSuffix ], match_units = energyUnits+tempUnits),\
        X4ColumnParser( match_labels = [ 'KT' + s for s in resolutionFWSuffix ], match_units = energyUnits+tempUnits + percentUnits, scale_factor = 0.5 )\
    ),\
    X4IndependentColumnPair(\
        X4ColumnParser( match_labels = [ 'KT' + s for s in variableSuffix ], match_units = energyUnits+tempUnits),\
        X4ColumnParser( match_labels = [ 'KT' + s for s in resolutionHWSuffix ], match_units = energyUnits+tempUnits + percentUnits )\
    ),\
    X4HighLowColumnPair( \
    ...
    X4MissingErrorColumnPair(\
        X4ColumnParser( match_labels = [ 'TEMP' + s for s in variableSuffix + shiftSuffix ] + [ 'KT' + s for s in variableSuffix ], match_units = energyUnits+tempUnits ),\
    ),\
]
...
csDataParserList   = [\
    X4IndependentColumnPair( \
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y,[ [ b + s for s in variableSuffix + frameSuffix ] for b in baseDataKeys ] ), match_units = crossSectionUnits + noUnits ),\
        X4ColumnParser( match_labels = [ b + '-ERR' for b in baseDataKeys ] + dataTotalErrorKeys, match_units = crossSectionUnits + noUnits + percentUnits ),\
    ),\
    X4HighLowColumnPair( \
    ...
    X4MissingErrorColumnPair(\
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix ] for b in baseDataKeys ] ), match_units = crossSectionUnits + noUnits ),\
    ),\
]

nubarParserList   = [\
    X4IndependentColumnPair( \
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y,[ [ b + s for s in variableSuffix ] for b in baseDataKeys ] ), match_units = nubarUnits + noUnits ),\
        X4ColumnParser( match_labels = [ b + '-ERR' for b in baseDataKeys ] + dataTotalErrorKeys, match_units = nubarUnits + noUnits + percentUnits ),\
    ),\
    X4HighLowColumnPair( \
...
angDistParserList = [\
    X4IndependentColumnPair( \
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseDataKeys ] ), match_units = angDistUnits + noUnits ),\
        X4ColumnParser( match_labels = [ b + '-ERR' for b in baseDataKeys ] + dataTotalErrorKeys, match_units = angDistUnits + noUnits + percentUnits ),\
    ),\
    X4MissingErrorColumnPair(\
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseDataKeys ] ), match_units = angDistUnits + noUnits ),\
    ),\
...
energyDistParserList = [\
    X4IndependentColumnPair( \
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseDataKeys ] ), match_units = energyDistUnits + noUnits ),\
        X4ColumnParser( match_labels = [ b + '-ERR' for b in baseDataKeys ] + dataTotalErrorKeys, match_units = energyDistUnits + noUnits + percentUnits ),\
    ),\
    X4MissingErrorColumnPair(\
        X4ColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseDataKeys ] ), match_units = energyDistUnits + noUnits ),\
    ),\
...
angleParserList   = [\
    X4IndependentColumnPair(\
        X4AngleColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseAngleKeys ] ), match_units = angUnits + noUnits ),\
        X4AngleColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in errorSuffix + resolutionHWSuffix ] for b in baseAngleKeys ] ), match_units = angUnits + noUnits + percentUnits )\
    ),\
    X4IndependentColumnPair(\
        X4AngleColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseAngleKeys ] ), match_units = angUnits + noUnits ),\
        X4AngleColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in resolutionFWSuffix ] for b in baseAngleKeys ] ), match_units = angUnits + noUnits + percentUnits, scale_factor = 0.5 )\
    ),\
    X4MissingErrorColumnPair(\
        X4AngleColumnParser( match_labels = reduce( lambda x, y: x + y, [ [ b + s for s in variableSuffix + frameSuffix ] for b in baseAngleKeys ] ), match_units = angUnits + noUnits ),\
    ),\
