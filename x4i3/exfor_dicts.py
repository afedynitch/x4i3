...
def getDictionary( filename, VERBOSELEVEL=0 ):
    if type( filename ) is not str: raise TypeError('Variable filename is supposed to be a string, got a '+str(type(filename)))
    try:
        f = open( filename )
    except IOError:
        if VERBOSELEVEL>0:print "Dictionary file "+filename+" not found"
        return None
    
    # Initialize variables (they should get overwritten in the exec() calls below)
    Title = ''
    FieldBreaks = []
    NumFields=0
    ...
    for line in f:
        ...
        if line != "":
            fieldlist = []
            for i in range(NumFields) : fieldlist.append(line[FieldBreaks[i]:FieldBreaks[i+1]].strip())
            item = []
            for i in range(1,NumFields): item.append(fieldlist[i])
            d[fieldlist[0]]=item
        FieldBreaks.pop()
    f.close()
    return d
...
class X4DictionaryServer:
    ...
    def getDictionaryFilename( self, x ):
        ...
        if type( x ) is int:
            for i in self.DictionaryNames :
                if ( i[0] == x ) : return "dict"+repr(i[0]).zfill(2)+".txt"
        elif type( x ) is str:
            for i in self.DictionaryNames :
                if ( i[1] == x ) : return "dict"+repr(i[0]).zfill(2)+".txt"
        return None
    ...
    def getDictionary( self, x, VERBOSELEVEL=0 ):
        ...
        return getDictionary( self.pathToDictionaryFiles+filename, VERBOSELEVEL=0 )
    ...
    def getAllDictionaries(self, VERBOSELEVEL=0):
        ...
        """
        if VERBOSELEVEL>1: print 'Loading EXFOR Dictionaries:'
        dictmap = {}
        for i in self.DictionaryNames:
