...
#------------------------------------------------------
VERBOSELEVEL=2
x4ReactionMap = {'TOT':'Total','EL':'Elastic','ABS':'Absorption','INL':'Inelastic','TCC':'TotalChargeChanging','FUS':'Fusion','NON':'Nonelastic','SCT':'Scattering','F':'Fission','PAI':'PairProduction','THS':'ThermalScattering','X':'Unspecified','0':'None'}
...
class X4Process:
    ...
    def __init__( self, xl, IgnoreIsomerMath = False ):
        if type( xl ) == str: xl = self.parse( xl ) 
        if not isinstance( xl, pyparsing.ParseResults ): raise TypeError( "__init__ takes pyparsing.ParseResults as argument, got " + str( type( x ) ) )
    ...
    def __str__( self ):  
        ...
        if self.residual : return result + str( self.residual )
        return result
        
    def __repr__( self ):                     
        result  = repr( self.targ ) + '(' + repr( self.proj ) + ','
        result += '+'.join( map( repr, self.products ) ) + ')'
        if self.residual : result += repr( self.residual )
        return result.replace( "'","" ).upper()
        
    def parse( self, x ): 
        try: return x4process.parseString( x )
        except pyparsing.ParseException, err: raise ReactionParsingError( 'Can not parse process "' + x + '",\n    got error "' + str( err ) + '"\n   ' )
        
    def setParticleType( self, i ):
        """Figures out what a particle i corresponds to, then creates it"""
        if isinstance( i, pyparsing.ParseResults ): il = i.asList()
        elif type( i ) == list: il = i
        elif type( i ) == str: 
            if i in ['ELEM','MASS','ELEM/MASS','X','NPART']: return X4VariableParticle( i )
            else:                                            return X4Particle( i )
        else : raise TypeError
        if len( il ) == 3:
            if il[2].isdigit():
                if il[2] == '0':   return X4Element( i )
                else :             return X4Nucleus( i )
        elif len( il ) > 3:        return X4Isomer( i, self.IgnoreIsomerMath )
        elif len( il ) == 2:       return X4Particle( i )
        else:
            if len( il ) == 1 and len( il[0] ) > 4:return X4ChemicalCompound( i[0] )
            else :                     return X4Particle( i )
    ...
    def setProduct( self, i ):
        ...
        if i in x4ReactionMap.keys(): return i 
    ...
    def setResidual( self ):
        ...
        if Zin == Zout and Ain == Aout: self.residual = self.products.pop( len( self.products ) - 1 )
        elif can_compute_residual:
            try: self.residual = X4Nucleus( [ str( Zin - Zout ), endl_ZSymbol( Zin - Zout ), str( Ain - Aout ) ] )
            except AttributeError:
                if   ( Zin - Zout ) == -1 and ( Ain - Aout ) == 0: self.residual = X4Particle( 'E' )
                elif ( Zin - Zout ) == +1 and ( Ain - Aout ) == 0: self.residual = X4Particle( 'B+' )
                elif abs( Zin - Zout ) > 1000 or abs( Ain - Aout ) > 1000: self.residual = None
                else: raise ResidualNucleusError( "Cannot compute residual, Delta Z = " + str( Zin - Zout ) + ", Delta A = " + str( Ain - Aout ) + ", for reaction " + str( self.parse_results ) )
        else : self.residual = None
        # remove residual from product list in case of double entries
        if self.residual in self.products: self.products.remove( self.residual )
...
class X4Reaction( X4Process, X4Measurement ):
    ...
    def parse( self, x ): 
        try: return x4reaction.parseString( x )
        except pyparsing.ParseException, err: 
            if "/" in x or "+" in x or "//" in x: raise IsomerMathParsingError( 'Reaction ' + x + ' looks like it contains "isomer math"' )
...
class X4ReactionCombination( X4Measurement ):
    ...
    def parse( self, x ): 
        try: return x4compound_expression.parseString( x )
        except pyparsing.ParseException, err: raise ReactionParsingError( 'Can not parse compound reaction expression "' + x + '",\n    got error "' + str( err ) + '"\n   ' )
        
    def getStrOrX4Reaction(self,i):
        if i in [ '(', ')', '+', '-', '*', '/', '=', '//' ]: return i
        else : 
            try: return X4Reaction( i )
            except ( IsomerMathParsingError ), err: return X4ReactionIsomerCombination( i )
            
    def getReactionList( self ):
        """assemble a list of all the reactions in self.data"""
        result = []
        for x in self.data:
            if isinstance( x, X4Reaction ): result.append(x)
            elif isinstance( x, X4ReactionIsomerCombination ): result += x.getReactionList()
        return result
        
    def has_quantity( self, i ):
        if len( self.reaction_list ) < 1: raise TypeError( "Reaction list too short in X4ReactionCombination" )
        for r in self.reaction_list:
            if r.has_quantity( i ): return True
        return False
...
class X4ReactionIsomerCombination( X4ReactionCombination ):
    ...
    def repack( self, x ): 
        ...
        # ... non-trivial quants ...
        quant = x[1]
        if ',' not in quant: quant=','+quant
        # ... possible isomer projectiles ...
        if len(uglyRxn[0])>3: targ = '-'.join( uglyRxn[0][0:3] )+'-'+''.join(uglyRxn[0][4:]) 
        else: targ = '-'.join( uglyRxn[0] )
        # ... maybe heavy-ion projectiles ...
        if type(uglyRxn[1]) == str: proj = uglyRxn[1]
        else: proj = '-'.join( uglyRxn[1] )
        # ... lots of products ...
        proc = []
        for protoProc in uglyRxn[2:-1]:
            if type( protoProc ) == str: proc.append( protoProc )
            else: proc.append( ''.join( protoProc ) )
        proc = '+'.join( proc ) 
        ...
        for thing in uglyResid[5:]:
            if thing in [ '-', '+', '=', '/', '//' ]: residList.append( thing )
            elif thing in [ '0','1','2','3','4','5']: residList[-1][-1]+=thing
            else: residList.append( residPrefix[0:-1]+[thing] )
...
if __name__=="__main__":
    ...
    for i in ["(N,2n)","(N,N)94-PU-240","(N,TOT)","(N,F)","(P,2p+4n)","(94-PU-240,X)",'(0,F)','(P,A+6-C-14+4N)','(P,A+6-C-14+4N+X)']: #has trouble with isomer targets
        ii="94-PU-240"+i
    ...
    for i in ["(94-PU-239(N,F),,SIG)","(92-U-238-M(0,F),TER,AKE,LCP)","(94-PU-239(N,ABS),,SIG,,MXW)","(94-PU-239(P,A+6-C-14+4N),,SIG)","(94-PU-240(N,0),,EN)"]:
    ...
    #    test(i)
        ...
        for j in x4compound_expression.parseString(i): print j
    #---------------------------------
    print
    ...
    for line in lines:
        ...
        else: lastfield+=' '+line[11:66].strip()
    rxnfieldlist.append(lastfield)
    rxnfieldlist.pop(0)
    ...
    for line in rxnfieldlist:
        ...
        if iline in pointers.keys(): print 'has pointer ',pointers[iline]
        print line
